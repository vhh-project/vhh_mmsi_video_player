...
key_frames_interval="$(echo `/usr/local/Cellar/ffmpeg/4.2.1/bin/ffprobe ${source} 2>&1 | grep -oE '[[:digit:]]+(.[[:digit:]]+)? fps' | grep -oE '[[:digit:]]+(.[[:digit:]]+)?'`*2 | bc || echo '')"
...
/usr/local/Cellar/ffmpeg/4.2.1/bin/ffmpeg ${misc_params} -i ${source} ${cmd}
